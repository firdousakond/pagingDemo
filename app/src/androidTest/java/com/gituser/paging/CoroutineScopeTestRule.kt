 ...
 * ```
 *
 * All coroutines started on [CoroutineScopeTestRule] must complete (including timeouts) before the test
 ...
 * ```
 *
 * You may call [DelayController] methods on [CoroutineScopeTestRule] and they will control the
 * virtual-clock.
 *
 * ```
 ...
 * ```
 *
 * By default, [CoroutineScopeTestRule] will be in a *resumed* state.
 *
 * @param dispatcher if provided, this [TestCoroutineDispatcher] will be used.
 */
@ExperimentalCoroutinesApi
class CoroutineScopeTestRule(private val dispatcher: TestCoroutineDispatcher = TestCoroutineDispatcher()) :
    TestWatcher(),
